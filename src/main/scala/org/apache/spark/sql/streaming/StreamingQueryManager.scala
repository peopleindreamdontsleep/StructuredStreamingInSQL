...
class StreamingQueryManager private[sql] (sparkSession: SparkSession) extends Logging {
  ...
  def remove(queryID:UUID): Unit ={
    activeQueriesLock.synchronized {
      println("befor size"+activeQueries.size)
      println("befor del"+queryID)
      println(activeQueries.keys.toString())
      activeQueries -= queryID
      println("after size"+activeQueries.size)
    }
  }
  ...
  private[sql] def startQuery(
    ...
    activeQueriesLock.synchronized {
      ...
      userSpecifiedName.foreach { name =>
        println("make sure name is not same")
